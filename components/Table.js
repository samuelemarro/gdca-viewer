...
export default function Table({ width, height, content, fontSize, setDisplayedTank }) {
    return (
            ...
            {Array.from({length: height}, (_, i) => (
                <tr suppressHydrationWarning key={i}>
                    ...
                    {Array.from({length: width}, (_, j) => (
                        <Cell key={j} content={content && content[i] && content[i][j]} fontSize={fontSize} setDisplayedTank={setDisplayedTank} />
                    ))}
                </tr>
            ))}
        </table>
